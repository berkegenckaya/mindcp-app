...
import { TrendingUp, TrendingDown, Minus, ExternalLink, Copy, Check, Droplets, Info, MousePointer } from 'lucide-react'
      ...
      <Card className="relative group rounded-2xl border border-white/35 bg-white/18 backdrop-blur-md shadow-[inset_0_0_0_1px_rgba(255,255,255,0.4),0_6px_22px_rgba(0,0,0,0.14)] transition-all duration-300 hover:bg-white/26">
        ...
        <CardContent className="space-y-3">
          {pools.slice(0, 5).map((pool, index) => (
            <div
              key={pool.id}
              className="rounded-xl bg-white/20 backdrop-blur-sm border border-white/30 p-4 hover:bg-white/30 transition-all duration-200 cursor-pointer hover:scale-[1.02] hover:shadow-lg"
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <Badge
                      variant="outline"
                      className="text-xs bg-purple-50/20 text-purple-700 border-purple-300/50 px-2 py-0.5"
                    >
                      {pool.network}
                    </Badge>
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="font-semibold text-gray-900 text-sm hover:text-purple-600 hover:underline transition-colors cursor-pointer"
                ...
                <div className="text-right flex-shrink-0">
                  <div className="font-semibold text-gray-900 text-sm">{pool.price}</div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs mb-3">
                <div className="bg-white/10 rounded-lg p-2">
                  ...
                  <div className="font-semibold text-gray-900">{pool.volume_24h}</div>
                </div>
                <div className="bg-white/10 rounded-lg p-2">
                  <div className="text-gray-600 font-medium mb-1">Liquidity</div>
                  <div className="font-semibold text-gray-900">{pool.liquidity}</div>
                </div>
                <div className="bg-white/10 rounded-lg p-2 col-span-2 sm:col-span-1">
                  ...
                  <div className="font-semibold text-gray-900">{pool.market_cap}</div>
                </div>
              </div>
